PKG_ID := $(shell ~/.local/bin/yq e '.["id"]' manifest.yaml)
PKG_VERSION := $(shell ~/.local/bin/yq e '.["version"]' manifest.yaml)
...
$(PKG_ID).s9pk: manifest.yaml instructions.md assets/haven-icon.svg LICENSE scripts/embassy.js docker-images/aarch64.tar docker-images/x86_64.tar
ifeq ($(ARCH),aarch64)
